...
namespace ServiceRest_085
{
    ...
    public interface ITI_UMY
    {
        ...
        [OperationContract]
        [WebGet(UriTemplate = "Mahasiswa", ResponseFormat = WebMessageFormat.Json)] //untuk membuat slash, selalu relative
        string UpdateMahasiswa(string nim, string nama, string prodi, string angkatan); //mendapatkan seluruh data mahasiswa

        [OperationContract]
        [WebGet(UriTemplate = "Mahasiswa/nim={nim}", ResponseFormat = WebMessageFormat.Json)] //untuk get
        string DeleteMahasiswaByNIM(string nim); //mengambil data berdasarkan nim

        [OperationContract]
        [WebInvoke(UriTemplate = "Mahasiswa", RequestFormat = WebMessageFormat.Json, ResponseFormat = WebMessageFormat.Json)]
        string CountMahasiswa();
        // TODO: Add your service operations here
    }

    // Use a data contract as illustrated in the sample below to add composite types to service operations.
    // You can add XSD files into the project. After building the project, you can directly use the data types defined there, with the namespace "ServiceRest_085.ContractType".
    [DataContract]
